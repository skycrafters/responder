...
const performAction = (action, flow) => {
	...
	if (action.command) {
		if (!fs.existsSync(path.resolve("./actions/", action.command))) {
			return Promise.reject(new Error(`Module ${action.command} does not exist`));
		}
		...
		if (
			!fs.existsSync(path.resolve(__dirname, "../rules/", provider, ruleName))
		) {
			return Promise.reject(new Error(`Module ${ruleName} does not exist`));
		}
		...
		if (action.rule === "check") {
			if (!rule.check) {
				return Promise.reject(new Error(`Check function ${ruleName} does not exist`));
			}
		...
		if (action.rule === "remediate") {
			if (!rule.remediate) {
				console.error("Remediate function is not set for %s %s", ruleName);
				return Promise.reject(new Error(`Remediate function ${ruleName} does not exist`));
			}
		...
		return Promise.reject(new Error("Undefined action to perform"));
	}
	...
	return run(action, flow)
		.then((result) => {
			...
			if (!result) {
				throw new Error(
					`${action.command || action.rule} did not return the expected value`
				);
			}
		})
		.catch((error) => {

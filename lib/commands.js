...
const performAction = (action, flow) => {
	...
	if (action.command) {
		...
		if (!fs.existsSync(path.resolve(__dirname, "../rules/", ruleName))) {
			return Promise.reject(`Module ${ruleName} does not exist`);
		}

		let rule = require(path.resolve(__dirname, "../rules/", ruleName));
...
module.exports.captureEvent = (event, ruleflow) => {
	if (!path.resolve(__dirname, "./rules/", ruleflow.rule)) {
		console.log(__dirname, "./rules/", ruleflow.rule);
		console.log("Rule %s not found", ruleflow.rule);
		throw new Error(`Rule ${ruleflow.rule} not found`);
	}

	const rule = require(path.resolve("./rules/", ruleflow.rule));

	return rule.capture(event).map((finding) => {
		finding.ruleName = ruleflow.rule;
		finding.triggeredAt = Date.now();

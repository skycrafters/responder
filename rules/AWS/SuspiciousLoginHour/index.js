...
rule.capture = (event, context) => {
	...
	if (operatingHours && operatingHours.timeZone) {
		if (isWeekend(operatingHours.timeZone) && operatingHours.weekend) {
			const now = moment.tz(operatingHours.timeZone);
			const from = moment(operatingHours.weekend.from, ["h:mmA"]).format(
				"HH:mm"
			);
			const to = moment(operatingHours.weekend.to, ["h:mmA"]).format("HH:mm");
			if (
				!now.isBetween(
					now
						.clone()
		...
		if (!isWeekend(operatingHours.timeZone) && operatingHours.week) {
			const now = moment.tz(operatingHours.timeZone);
			const from = moment(operatingHours.week.from, ["h:mmA"]).format("HH:mm");
			const to = moment(operatingHours.week.to, ["h:mmA"]).format("HH:mm");
			if (
				!now.isBetween(
					now
						.clone()
